`timescale 1ns / 1ps
// to collect the bits coming fom relu one by one of size 12 bit , 144 pixels 
module conv2_buf
    #(
    ...
    (
        ...
        output reg  [DATA_BITS-1:0]   data_out [0:24],  // given to data_in of conv2 module 
        output reg                    valid_out_buf
    );

    // Fixed architecture parameter for a 5x5 convolution
    localparam FILTER_SIZE = 5;
    localparam BUF_SIZE    = WIDTH * FILTER_SIZE;  //12*5 = 60 
    ...
    reg [2:0]                  buf_flag;  // Circular row tracker (0 to 4) , 000 , 001 , 010 , 011 , 100 ; 
